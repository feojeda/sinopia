function renderAntigravitySkill(capability, instructions) {
  ...
  const triggerList = Array.isArray(capability.triggers) ? capability.triggers.join(' or ') : '';
  const descriptionText = triggerList
    ? `Use when the user invokes ${triggerList} or asks to use ${capability.description}`
