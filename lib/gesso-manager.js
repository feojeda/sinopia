      ...
      if (fs.existsSync(lienzoJsonPath)) {
        try {
          ...
          lienzos.push({
            ...
            linkedPlanId: lienzoData.linkedPlanId || null
          });
        } catch (e) {
...
async function linkPlan(lienzoId, planId) {
  const lienzoDir = findLienzoDir(lienzoId);
  const lienzoJsonPath = path.join(lienzoDir, 'lienzo.json');
  const gessoMdPath = path.join(lienzoDir, 'gesso.md');
  ...
  try {
    ...
    // Idempotency: same lienzo to same plan (skip state/hash validation)
    if (lienzoData.linkedPlanId === planId) {
      ...
      if (planDir) {
        ...
        if (fs.existsSync(planJsonPath)) {
          ...
          // Crash-recovery repair: if plan is missing the back-link, repair it
          if (planData.sourceLienzoId !== lienzoId) {
            const nowStr = new Date().toISOString();
            planData.sourceLienzoId = lienzoId;
            if (!planData.history) {
              planData.history = [];
            }
            const alreadyRecorded = planData.history.some(h =>
              h.action === 'linked-gesso' && h.details === `Vinculado desde lienzo ${lienzoId}`
            );
            if (!alreadyRecorded) {
              planData.history.push({
                timestamp: nowStr,
                action: 'linked-gesso',
                details: `Vinculado desde lienzo ${lienzoId}`
              });
            }
    ...
    // Lienzo already linked to another plan
    if (lienzoData.linkedPlanId) {
      const error = new Error(`El lienzo ${lienzoId} ya está vinculado al plan ${lienzoData.linkedPlanId}.`);
      error.code = 'GSDC_GESSO_LINK_FAILED';
      error.exitCode = 36;
      throw error;
    }

    if (lienzoData.status !== 'gesso_listo') {
      const error = new Error(`El lienzo no está en estado 'gesso_listo'. Estado actual: '${lienzoData.status}'.`);
      error.code = 'GSDC_GESSO_INVALID_STATE';
      error.exitCode = 32;
      throw error;
    }
    ...
    if (!lienzoData.confirmation || !lienzoData.confirmation.confirmed || !lienzoData.confirmation.gessoHash) {
      const error = new Error(`El lienzo ${lienzoId} no tiene una confirmación válida.`);
      error.code = 'GSDC_GESSO_INVALID_STATE';
      error.exitCode = 32;
      throw error;
    }
    ...
    const planJsonPath = path.join(planDir, 'plan.json');
    if (!fs.existsSync(planJsonPath)) {
      const error = new Error(`El plan ${planId} no está correctamente inicializado.`);
      error.code = 'GSDC_GESSO_LINK_FAILED';
      error.exitCode = 36;
      throw error;
    }
    ...
    if (planData.status === 'delivered') {
      const error = new Error(`El plan ${planId} ya fue entregado y no puede vincularse.`);
      error.code = 'GSDC_GESSO_LINK_FAILED';
      error.exitCode = 36;
      throw error;
    }

    if (planData.sourceLienzoId && planData.sourceLienzoId !== lienzoId) {
      const error = new Error(`El plan ${planId} ya está vinculado al lienzo ${planData.sourceLienzoId}.`);
      error.code = 'GSDC_GESSO_LINK_FAILED';
      error.exitCode = 36;
      throw error;
    }
    ...
    lienzoData.linkedPlanId = planId;
    lienzoData.status = 'con_mockup';
    lienzoData.timestamps.updated = nowStr;
    lienzoData.history.push({
      timestamp: nowStr,
      action: 'link-plan',
      details: `Vinculado al plan ${planId}`
    });
    writeAtomicJson(lienzoJsonPath, lienzoData);

    // Only write plan.json if sourceLienzoId is actually changing (crash-recovery guard)
    if (planData.sourceLienzoId !== lienzoId) {
      planData.sourceLienzoId = lienzoId;
      if (!planData.history) {
        planData.history = [];
