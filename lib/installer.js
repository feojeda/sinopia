...
# <<< gsd-canva <<<`;
  ...
  if (currentManifest && currentManifest.agents) {
    for (const [agentId, agentRecord] of Object.entries(currentManifest.agents)) {
      if (!manifestAgents[agentId] && agentRecord.files && agentRecord.files.length > 0) {
        manifestAgents[agentId] = agentRecord;
        for (const f of agentRecord.files) {
          if (!manifestFilesList.some(mf => mf.target === f.target)) {
            manifestFilesList.push({ ...f });
          }
        }
      }
    }
...
async function upgrade(options = {}) {
  ...
  if (manifest.agents) {
    ...
    const capMap = new Map(capabilities.map(c => [c.capability.id, c]));
    const officialPaths = getOfficialAgentArtifactPaths();
    ...
    for (const [agentId, agentRecord] of Object.entries(manifest.agents)) {
      ...
      const managedEntries = (agentRecord.files || []).filter(f => f.managed);

      for (const fileEntry of managedEntries) {
        ...
        if (agMatch) capId = agMatch[1];
        else if (cmdMatch) capId = cmdMatch[2];
        if (!capId) continue;
        ...
        if (fs.existsSync(fullPath)) {
          ...
          if (fileEntry.sha256 && fileEntry.sha256 !== currentHash) {
            if (options.adopt) {
              fileEntry.sha256 = currentHash;
              if (manifest.files) {
                const flatEntry = manifest.files.find(mf => mf.target === fileEntry.target);
                if (flatEntry) flatEntry.sha256 = currentHash;
              }
        ...
        if (manifest.files) {
          const flatEntry = manifest.files.find(mf => mf.target === fileEntry.target);
          if (flatEntry) flatEntry.sha256 = newHash;
        }
      ...
      if (options.adopt || options.forceAll) {
        for (const { capability, instructions } of capabilities) {
          const targets = adapter.getTargets(capability);
          for (const { targetPath } of targets) {
            const alreadyManaged = managedEntries.some(f => f.target === targetPath);
            if (alreadyManaged) continue;

            const fullPath = path.join(targetDir, targetPath);
            if (!fs.existsSync(fullPath)) continue;

            const relTarget = targetPath;
            if (options.adopt) {
              const currentHash = getSha256(fullPath);
              const entry = {
                source: `agent-source:${capability.id}`,
              ...
              if (!agentRecord.files) agentRecord.files = [];
              agentRecord.files.push(entry);
              if (manifest.files && !manifest.files.some(mf => mf.target === relTarget)) {
                manifest.files.push({ ...entry });
              }
              updatedFiles.push(relTarget);
            } else if (options.forceAll && officialPaths.includes(relTarget)) {
              const capData = capMap.get(capability.id);
              if (!capData) continue;
              if (fs.existsSync(fullPath)) {
                const timestamp = new Date().toISOString().replace(/[-:T]/g, '').slice(0, 14);
