...
async function init(options = {}) {
  ...
  if (options.agent && options.agent !== 'antigravity') {
    const supported = agentAdapters.getSupportedAgents();
    if (!supported.includes(options.agent)) {
      const error = new Error(`Unsupported agent: '${options.agent}'. Supported agents: ${supported.join(', ')}`);
  ...
  if (options.agent === 'antigravity') {
    ...
    for (const { capability, instructions } of capabilities) {
      ...
      for (const { targetPath } of targets) {
        ...
        const rendered = adapter.render(capability, instructions);
      ...
      for (const skillDir of skillDirs) {
        ...
        if (fs.existsSync(skillFile)) {
          ...
          if (content.trim().length > 0 && content.includes('---')) {
            ...
            missingSkills.push(skillDir);
          }
        } else {
          missingSkills.push(skillDir);
        }
      }
      agentDetails.skillsDir = true;
      agentDetails.skillsCount = skillDirs.length;
      agentDetails.validSkills = validSkills;
      if (missingSkills.length > 0) {
        agentDetails.missingSkills = missingSkills;
      }
    } else {
      agentDetails.skillsDir = false;
    }
  }
