...
async function doctor(options = {}) {
  ...
  if (options.agent) {
    ...
    if (!agentDetails.allOfficialSkillsPresent) {
      const error = new Error(
        `Antigravity 2.0 skills validation failed. Missing: ${missingSkills.join(', ')}`
      );
      error.code = 'GSDC_AGENT_SKILLS_MISSING';
      error.exitCode = 19;
      error.details = { missingSkills, validSkills, expectedSkills: expectedSkillIds.length };
      throw error;
    }
  }
