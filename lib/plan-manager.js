...
async function confirmDecisions(planId, options = {}) {
  ...
  try {
    ...
    const fields = ['vertical', 'audiencia', 'formato', 'paleta', 'copy', 'cta'];
    const missing = [];
    ...
    // Calcular hash criptográfico de integridad canónico normalizado (Unicode NFC sensible a mayúsculas)
    const normalize = (val) => String(val || '').trim().normalize('NFC');
