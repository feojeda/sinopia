...
async function submitMockup(planId) {
  ...
  try {
    ...
    const mockupStats = fs.statSync(mockupHtmlPath);
...
async function resetConfirmation(planId) {
  ...
  try {
    ...
    // Validar estado: solo permitir reset desde ready_for_html o pending_approval
    const allowedStatuses = ['ready_for_html', 'pending_approval'];
    if (allowedStatuses.includes(planData.status)) {
      // Estado válido para reset
    } else if (planData.status === 'questions_pending' && decisions.confirmation.confirmed === false) {
      // Idempotente: ya está en questions_pending sin confirmar
      return {
        planId,
        reset: true,
      ...
      const error = new Error(`Transición ilegal: Solo se puede resetear desde mockup:ready_for_html o mockup:pending_approval. Estado actual: ${planData.phase}:${planData.status}`);
      error.code = 'GSDC_INVALID_STATE';
      error.exitCode = 13;
      throw error;
    }
